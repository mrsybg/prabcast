...
def plot_abc_xyz(data, aggregation_level, start_date, end_date,a_threshold,b_threshold, x_threshold, y_threshold):
    """
    ...
    if not st.session_state.get("selected_products_in_data"):
        st.error("Keine Produktspalten ausgewählt. Bitte wählen Sie Produkte in der Datenansicht aus.")
        return

    # Reshape die Daten von breit nach lang
    selected_products = st.session_state.selected_products_in_data
    long_df = data.melt(
        id_vars=[st.session_state.date_column],
    ...
    if 'Produkt' not in long_df.columns:
        st.error("Die Spalte 'Produkt' fehlt nach dem Melting der Daten. Überprüfen Sie die Produktspalten.")
        return

    # Filtere den Zeitraum
    long_df[st.session_state.date_column] = pd.to_datetime(long_df[st.session_state.date_column])
    filtered_data = long_df[
        (long_df[st.session_state.date_column] >= pd.to_datetime(start_date)) & 
        (long_df[st.session_state.date_column] <= pd.to_datetime(end_date))
    ]

    # Aggregation setup
    freq_map = {'Wöchentlich': 'W', 'Monatlich': 'M', 'Quartalsweise': 'Q'}
    freq = freq_map.get(aggregation_level, 'M')  # Standard auf 'Monatlich'
    ...
    try:
        sales_continuity = (
            filtered_data.set_index(st.session_state.date_column)
                       .groupby('Produkt')
        ...
        st.error(f"Fehler bei der Berechnung der Beständigkeit: {e}")
        return
...
def display_tab():
    ...
    start_date, end_date = st.date_input(
        ...
        value=[
            st.session_state.df[st.session_state.date_column].min(),
            st.session_state.df[st.session_state.date_column].max()
        ],
        min_value=st.session_state.df[st.session_state.date_column].min(),
        max_value=st.session_state.df[st.session_state.date_column].max()
    )
    ...
    if b_threshold <= a_threshold:
        st.error("B-Kategorie muss höher sein als A-Kategorie.")
    ...
    if y_threshold >= x_threshold:
        st.error("X-Kategorie muss höher sein als Y-Kategorie.")
