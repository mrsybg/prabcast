...
def display_tab():
    if not st.session_state.get('ready_for_processing', False):
    ...
    if local_product:
        try:
            fig = zerlegung(local_product, start_date, end_date)
            st.plotly_chart(fig)
        except Exception as e:
            st.error(f"Fehler bei der Zerlegung: {str(e)}")


def zerlegung(selected_product_local, start_date_local, end_date_local):
    ...
    fig.add_trace(
        go.Scatter(
            x=data[st.session_state.date_column],
