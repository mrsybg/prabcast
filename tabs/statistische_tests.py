...
def display_tab():
    ...
    # Produktauswahl
    selected_product_st_test = st.selectbox("Wähle ein Produkt für die Tests", st.session_state.selected_products_in_data)
    df_for_test = st.session_state.df[selected_product_st_test]
    ...
    with col1:
        ...
        try:
            ...
            st.error(f"Fehler beim Durchführen des ADF-Tests: {e}")
    ...
    with col2:
        ...
        try:
            ...
            st.error(f"Fehler beim Durchführen des KPSS-Tests: {e}")
...
def interpret_p_value(p_value, test_type="Test"):
    ...
    """
    if p_value < 0.05:
        st.success(f"P-Wert ist klein (weniger als 0.05). Die Nullhypothese des {test_type} wird abgelehnt.")
        if test_type == "ADF-Test":
            st.info("Das bedeutet, dass die Zeitreihe stationär ist.")
        elif test_type == "KPSS-Test":
            st.info("Das bedeutet, dass die Zeitreihe nicht stationär ist.")
    else:
        st.warning(f"P-Wert ist groß (mehr als 0.05). Die Nullhypothese des {test_type} kann nicht abgelehnt werden.")
        if test_type == "ADF-Test":
            st.info("Das bedeutet, dass die Zeitreihe nicht stationär ist.")
        elif test_type == "KPSS-Test":
            st.info("Das bedeutet, dass die Zeitreihe stationär ist.")